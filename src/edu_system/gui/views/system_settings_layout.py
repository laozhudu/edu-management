...
class SystemSettingsViewMixin:
    ...
    def _add_standard_title(self, layout: QVBoxLayout, title_text: str):
        ...
        title.setStyleSheet(
            f"color: {self.TITLE_COLOR}; margin-bottom: {self.TITLE_MARGIN_BOTTOM};"
        )
    ...
    def _add_standard_bottom_buttons(self, layout: QVBoxLayout, buttons: list | None = None):
        """添加标准底部按钮区"""
...
def setup_system_settings_view(
    view_instance: QWidget,
    title_text: str,
    tabs_config: list[tuple[str, str]],
    bottom_buttons: list | None = None,
):
    """
    统一配置系统设置视图
