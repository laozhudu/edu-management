...
class Updater(QObject):
    ...
    def __init__(self, current_version: str, repo_owner: str = "laozhudu", repo_name: str = "edu-management"):
        super().__init__()
            ...
            if self._is_newer(latest_version, self.current_version):
                ...
                if exe_asset:
                    self.check_finished.emit(True, latest_version, 
                        ...
                        exe_asset["browser_download_url"], exe_asset.get("size", 0))
                else:
    ...
    def download_update(self, download_url: str, progress_cb: Optional[Callable[[int], None]] = None) -> str:
        """下载更新包，返回本地路径"""
        self._cancel = False
...
class UpdateManager:
    ...
    def on_update_available(self, callback: Callable[[str, str], None]):
        ...
        self.updater.check_finished.connect(
            lambda success, version, msg, *args: 
            callback(version, msg) if success else None
        )
...
if __name__ == "__main__":
    ...
    updater = Updater(v)
    updater.check_finished.connect(
        lambda s, v, m, *args: print(f"检查结果: {s} {v} {m}")
    )
