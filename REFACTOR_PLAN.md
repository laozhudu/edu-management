...
| 网页端 | 纯 API（无前端） | **明确定位为 API 服务层**；Web 前端列为可选 Phase 5（YAGNI，不预设） | 避免虚构需求（Sprint 3.7.9 教训） |
...
### 3.4 未来 Web 前端扩展点（Phase 5，可选）

```
GUI 配置驱动 ──→ ui_config.json ──→ /api/config (REST)
                                        ↓
                              未来 Web 前端（Vue/React 或简单 HTML+JS）
                              复用同一配置，双端视觉一致
```
> 原则：**不在 v3.0 实现 Web 前端**（YAGNI）；仅保证 API 层与配置层为未来 Web 预留接口。
...
### Phase 5：Web 前端扩展（可选，v3.1 起）
- [ ] P5-1 评估需求：确认是否需要独立 Web 前端（当前 API 已可被任何 HTTP 客户端消费）
- [ ] P5-2 若需要：基于 /api/config + REST 构建轻量 Web UI（复用 ui_config 双端一致）
- [ ] P5-3 双端同步验证：桌面与 Web 渲染同一配置
