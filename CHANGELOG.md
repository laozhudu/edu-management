...
**界面层面（用户感知的减法）：**
- 侧栏页签 28→19（-9，减 32%）：系统设置 9→3、考试管理 4→2、报表工具 3→2、学生管理 4→3
- 消除"侧栏页签→页面内Tab→窗口标题"三层标签冗余

**代码层面（system_config.py 重构）：**
- 6 Tab 拍平为垂直卡片堆叠（1061→~1049 行，结构更清晰）
- 统一区块容器 `_make_section()` 消除重复布局代码
- 6 个区块各自独立：服务管理、定时任务、存储管理、网络设置、界面样式、操作审计
- 类型标注修复，mypy strict 仅剩 1 处基类 Any（无破坏性）

**规范化验收：**
- mypy strict 仅 1 错误（BaseView 类型 Any，框架层面）
- ruff 全绿
- GUI 全量 66 passed 零回归
...
**U0 全局基础：**
- 引入 FontAwesome 6.5 图标字体（全站可用 fa 图标）
- 若依蓝 primary 色板 #409EFF + Element 语义色（#67c23a/#e6a23c/#f56c6c）

**U1 登录页重设计：**
- 渐变蓝紫背景 + 装饰光斑 + 玻璃拟态卡片
- 品牌 logo 图标（毕业帽）+ 图标输入框 + 渐变登录按钮 + 加载态

**U2 主框架美化：**
- 顶栏：校名渐变 logo + 图标按钮（fa-bars/fa-moon）
- 侧栏：域 emoji 图标 + 页签箭头 icon，移除失效 SVG

**U3 通用组件质感（base.css 全局增强）：**
- 卡片：柔和双层阴影 + 悬停提升 + 圆角
- 按钮：主按钮渐变 + 悬停抬升 + 微交互
- 表格：表头渐变 + 行悬停高亮 + 圆角容器
- 表单：聚焦光晕环；徽章统一色彩；label/空状态/页标题标准化

**验收：** 73 passed（页面契约全绿），全量 661 基线

## [4.1.0] - 2026-08-10

### 底座复用实证：图书管理演示业务域（不局限教务）

**A2 演示业务域（"自己的诺依"最终证明）：**
- 新增独立业务域 library：Book/BorrowRecord model + 图书 CRUD + 借还闭环
- 完全复用现有底座：RBAC 权限、PageQuery 分页、统一返回、自动审计、依赖注入
- 审计扩展点：AUDIT_TABLES 加 books/borrow_records/departments（新业务域一行接入审计）
- Web library.html（图书列表 + 借阅记录 + 借还操作）+ 浏览器实测通过
- ui_config 加"图书管理"新域（配置化挂载，10 域）+ 契约更新
- 契约 +9（含审计集成验证）

**意义：证明底座（core/repository/审计/权限/分页）与业务解耦，可挂载任何新业务域。**
