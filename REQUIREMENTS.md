...
6. **双端定位**：桌面 PyQt5 = 主 UI；API FastAPI = 服务层；**Web 前端 = 与桌面功能完全一致**（双端同功能、同配置、同权限，2026-08-04 用户定案）
...
### G. 双端一致 + 快捷验收（用户 2026-08-04 补充 ⭐ 2026-08-04 升级）
> 用户定位：桌面端 + 网页端**都要**，**两端功能完全一致**（2026-08-04 定案：Web 非简化版/非只读，功能与桌面完全相同），同一 ui_config.json 驱动界面，桌面快捷方式方便实时验收。技术栈后续单定。

| # | 需求 | 产出 | 验收标准 |
|---|------|------|----------|
| G1 | **桌面快捷方式**：Linux .desktop + Windows 快捷方式，双击直达主界面 | 打包/安装脚本 | 桌面双击即启动 |
| G2 | **Web 端功能与桌面完全一致**：基于 ui_config.json 渲染 6 域全部页签（26 个），业务功能逐项对等（见下方功能对等清单） | `src/edu_system/static/` + 前端应用 | 浏览器访问 / → 6 域 26 页签完整可用，与桌面功能逐项对等 |
...
| G6 | **双端功能对等验收**：桌面 18 视图 ↔ Web 页面逐项映射，功能差异为零（含报表导出/导入向导/动态字段/主题切换等） | 功能映射表 + 端到端验收 | 映射表 100% 覆盖，无缺失功能 |

**双端功能对等清单（Web 必须逐项实现，源自 ui_config.json 6 域 + 桌面 18 视图）**：

| 域 | 页签/功能 | Web 对等要求 |
|----|----------|-------------|
| home 首页 | 学期概览 / 快捷操作 / 待办·数据状态 | 仪表盘、快捷入口、待办角标与桌面一致 |
| students 学生管理 | 学生信息（CRUD/查询/动态字段/分页） / 新生注册 / 学籍变动 / 升留级·毕业 | 全量对等：含 ext_json 动态字段表单/表格/导入导出 |
| scores 成绩管理 | 成绩录入（含粘贴/Excel 导入/排名/锁定） / 成绩查询 / 成绩统计 / 排名分析 | 全量对等：粘贴录入、折算分、锁定联动 |
| exams 考试管理 | 考试管理 / 考场座位 / 监考安排 / 准考证 | 全量对等 |
| teachers 教师管理 | 教师信息 / 任课安排 | 全量对等 |
| system 系统设置 | 学期设置 / 班级科目 / 教室位置 / 用户权限 / 数据维护 / 系统设置 / 初始化系统 | 全量对等：含配置继承四色预览、数据锁定 |
| 全局能力 | 登录/角色权限 / 主题亮暗切换 / 学期切换 / 配置热加载 / 报表导出(Word/Excel/PDF) / 导入向导 / 服务管理 | 全量对等：报表下载、批量操作与桌面一致 |
...
| ~~Web 动态渲染（templates/static 空目录）~~ | **已撤销（2026-08-04）**：Web 端升级为与桌面功能完全一致（见 G 组），不再 YAGNI |
